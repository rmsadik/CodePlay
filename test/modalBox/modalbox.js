...
Modalbox.Methods = {
	...
	loadContent: function() {
		if (this.event("beforeLoad") != false) { // If callback passed false, skip loading of the content
			...
			//added here JT, pass in the string 'loading' to simply display the spinning icon with the title bar
			...
			if (typeof this.content == 'string') {
				...
				{ // URL given as a parameter. We'll request it via Ajax
	...
	maxWindow: function() {
		...
		tmp.viewport = document.viewport.getDimensions(); 
		//Gets the current modalbox height and width
		tmp.modalbox = {'width': $(this.MBwindow).getWidth(), 'height': $(this.MBwindow).getHeight()};
		this.resize((tmp.viewport.width * 1 - tmp.modalbox.width * 1), tmp.viewport.height);
	}
};
